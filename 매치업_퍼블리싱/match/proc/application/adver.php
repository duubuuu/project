...
$uploadBase = G5_PATH."/match/upload_img/adver/";
        ...
        if(in_array($ext,$valid_formats))
        {
            if($size < ( 10*1024*1024 )) // Image size max 1 MB
            {
                ...
                if(move_uploaded_file($tmp, $uploadBase.$actual_image_name))
                {
                  ...
                  $sql = "insert into adver_ent(title, belong_to, name, email, word, img_name, img_path, img_size, reg_date)
                          value('{$title}', '{$belong_to}', '{$name}', '{$email}', '{$word}', '{$img_name}', '{$img_path}', '{$img_size}', now())";
                  ...
                  if($state){
                    ...
                    $subject = $title." [".$sug_name."] 님의 광고신청 입니다.";
